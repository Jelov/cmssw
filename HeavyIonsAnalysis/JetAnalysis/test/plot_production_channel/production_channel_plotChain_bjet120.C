...
void production_channel_plotChain_bjet120(){
...
//		t1->Add("/mnt/hadoop/store/user/chengchi/Pythia8_bjet30_5020GeV_GEN-SIM/MC_bJet30_HiForest/160221_200407/0000/HiForestAOD_pp_MC_bjets30_*.root");
		t1->Add("/mnt/hadoop/store/user/chengchi/Pythia8_bjet120_5020GeV_GEN-SIM/MC_bJet120_HiForest/160221_200452/0000/HiForestAOD_pp_MC_bjets120_*.root");
...
//	TH1F *h_qJet_dR_flavormatched = (TH1F*)f1->Get("ak4PFJetAnalyzer/h_qJet_dR_flavormatched");
  ...
  Canvas_channel->SaveAs("plots/bjet120/production_channel.pdf");
  ...
  int n_channels=16;
	...
	TH1D* bjet120_channel_pt[n_channels];  	
  TH1D* bjet120_channel_pt_eta2less[n_channels];
	TH1D* bjet120_channel_eta[n_channels];
	TH1D* bjet120_channel_dphi[n_channels];

	TCanvas *C_bjet120_channel_pt[n_channels];
  TCanvas *C_bjet120_channel_pt_eta2less[n_channels];	
	TCanvas *C_bjet120_channel_eta[n_channels];
	TCanvas *C_bjet120_channel_dphi[n_channels];	

	
	for (int i_channels=1; i_channels<n_channels;++i_channels){

		///////// bjet120_channel_pt
		bjet120_channel_pt[i_channels] = new TH1D( Form("bjet120_channel_pt_%i",i_channels) , Form("bjet120_channel_%i_pt",i_channels) ,ptBins,ptLo,ptHi);
		C_bjet120_channel_pt[i_channels] = new TCanvas( Form("C_bjet120_channel%i_pt",i_channels) , Form("C_bjet120_channel%i_pt",i_channels) );
  	t1->Draw( Form("jtpt>>bjet120_channel_pt_%i",i_channels) , Form("refjet_prodChannel==%i",i_channels) );
		bjet120_channel_pt[i_channels]->GetXaxis()->SetTitle("jet_pt");
		bjet120_channel_pt[i_channels]->Draw();
  	cout<<Form("jtpt>>bjet120_channel_pt[%i]",i_channels)<<endl;
		C_bjet120_channel_pt[i_channels]->SaveAs( Form("plots/bjet120/bjet120_channel%i_pt.pdf", i_channels));
		C_bjet120_channel_pt[i_channels]->Close();


    ///////// bjet120_channel_pt_eta2less	
    bjet120_channel_pt_eta2less[i_channels] = new TH1D( Form("bjet120_channel_pt_eta2less_%i",i_channels) , Form("bjet120_channel_%i_pt_eta2less",i_channels) ,ptBins,ptLo,ptHi);
    C_bjet120_channel_pt_eta2less[i_channels] = new TCanvas( Form("C_bjet120_channel%i_pt_eta2less",i_channels) , Form("C_bjet120_channel%i_pt_eta2less",i_channels) );
    t1->Draw( Form("jtpt>>bjet120_channel_pt_eta2less_%i",i_channels) , Form("refjet_prodChannel==%i && abs(jteta)<2",i_channels) );
    bjet120_channel_pt_eta2less[i_channels]->GetXaxis()->SetTitle("jet_pt");
    bjet120_channel_pt_eta2less[i_channels]->Draw();
    cout<<Form("jtpt>>bjet120_channel_pt_eta2less[%i]",i_channels)<<endl;
    C_bjet120_channel_pt_eta2less[i_channels]->SaveAs( Form("plots/bjet120/bjet120_channel%i_pt_eta2less.pdf", i_channels));
		C_bjet120_channel_pt_eta2less[i_channels]->Close();

    ///////// bjet120_channel_eta
    bjet120_channel_eta[i_channels] = new TH1D( Form("bjet120_channel_eta_%i",i_channels) , Form("bjet120_channel_%i_eta",i_channels) ,etaBins,etaLo,etaHi);
    C_bjet120_channel_eta[i_channels] = new TCanvas( Form("C_bjet120_channel%i_eta",i_channels) , Form("C_bjet120_channel%i_eta",i_channels) );
    t1->Draw( Form("jteta>>bjet120_channel_eta_%i",i_channels) , Form("refjet_prodChannel==%i",i_channels) );
    bjet120_channel_eta[i_channels]->GetXaxis()->SetTitle("jet_eta");
    bjet120_channel_eta[i_channels]->Draw();
    cout<<Form("jteta>>bjet120_channel_eta[%i]",i_channels)<<endl;
    C_bjet120_channel_eta[i_channels]->SaveAs( Form("plots/bjet120/bjet120_channel%i_eta.pdf", i_channels));
		C_bjet120_channel_eta[i_channels]->Close();

   ///////// bjet120_channel_dphi
    bjet120_channel_dphi[i_channels] = new TH1D( Form("bjet120_channel_dphi_%i",i_channels) , Form("bjet120_channel_%i_dphi",i_channels) ,dphiBins,dphiLo,dphiHi);
    C_bjet120_channel_dphi[i_channels] = new TCanvas( Form("C_bjet120_channel%i_dphi",i_channels) , Form("C_bjet120_channel%i_dphi",i_channels) );
    t1->Draw( Form("ref_qTwin_dphi>>bjet120_channel_dphi_%i",i_channels) , Form("refjet_prodChannel==%i",i_channels) );
    bjet120_channel_dphi[i_channels]->GetXaxis()->SetTitle("#Delta#phi");
    bjet120_channel_dphi[i_channels]->Draw();
    cout<<Form("ref_qTwin_dphi>>bjet120_channel_dphi[%i]",i_channels)<<endl;
    C_bjet120_channel_dphi[i_channels]->SaveAs( Form("plots/bjet120/bjet120_channel%i_dphi.pdf", i_channels));
		C_bjet120_channel_dphi[i_channels]->Close();

	}


	TCanvas * C_bjet120_channelAll_pt = new TCanvas("C_bjet120_channelAll_pt","C_bjet120_channelAll_pt");
	C_bjet120_channelAll_pt->Divide(4,4);
	for (int i_channels=1; i_channels<n_channels;++i_channels){
//    bjet120_channel_pt[i_channels] = new TH1D( Form("bjet120_channel_pt_%i",i_channels) , Form("bjet120_channel_%i_pt",i_channels) ,ptBins,ptLo,ptHi);
		C_bjet120_channelAll_pt->cd(i_channels);
//	t1->Draw( Form("jtpt>>bjet120_channel_pt_%i",i_channels) , Form("refjet_prodChannel==%i",i_channels) );
		bjet120_channel_pt[i_channels]->Draw();
	}
	C_bjet120_channelAll_pt->SaveAs("plots/bjet120/bjet120_channelAll_pt.pdf");


  TCanvas * C_bjet120_channelAll_pt_eta2less = new TCanvas("C_bjet120_channelAll_pt_eta2less","C_bjet120_channelAll_pt_eta2less");
  C_bjet120_channelAll_pt_eta2less->Divide(4,4);
  for (int i_channels=1; i_channels<n_channels;++i_channels){
    C_bjet120_channelAll_pt_eta2less->cd(i_channels);
    bjet120_channel_pt_eta2less[i_channels]->Draw();
  }
  C_bjet120_channelAll_pt_eta2less->SaveAs("plots/bjet120/bjet120_channelAll_pt_eta2less.pdf");


  TCanvas * C_bjet120_channelAll_eta = new TCanvas("C_bjet120_channelAll_eta","C_bjet120_channelAll_eta");
  C_bjet120_channelAll_eta->Divide(4,4);
  for (int i_channels=1; i_channels<n_channels;++i_channels){
    C_bjet120_channelAll_eta->cd(i_channels);
    bjet120_channel_eta[i_channels]->Draw();
  }
  C_bjet120_channelAll_eta->SaveAs("plots/bjet120/bjet120_channelAll_eta.pdf");


  TCanvas * C_bjet120_channelAll_dphi = new TCanvas("C_bjet120_channelAll_dphi","C_bjet120_channelAll_dphi");
  C_bjet120_channelAll_dphi->Divide(4,4);
  for (int i_channels=1; i_channels<n_channels;++i_channels){
    C_bjet120_channelAll_dphi->cd(i_channels);
    bjet120_channel_dphi[i_channels]->Draw();
  }
  C_bjet120_channelAll_dphi->SaveAs("plots/bjet120/bjet120_channelAll_dphi.pdf");
